...
abstract class Connection
{
    ...
    protected function trigger($sql, $runtime, $explain = [], $master = false)
    {
        if (!empty(self::$event)) {
            ...
            // if (!preg_match('/FROM `fa_admin_nav`|FROM `fa_auth_rule`|FROM `fa_auth_group_access`/i', $sql)) {
            //     file_put_contents('debug_sql.txt', date('Y-m-d H:i:s') . '   LOG_WARNING!!!   ' . $sql . ' [ ' . $master . 'RunTime:' . $runtime . 's ]' . PHP_EOL, FILE_APPEND);
            // }
