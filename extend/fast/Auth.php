...
class Auth
{
    ...
    public function getRuleList($uid)
    {
        ...
        //读取用户组所有权限规则
        $this->rules = Db::name($this->config['auth_rule'])->where($where)->field('id,pid,ico,name,mca,ismenu')->select();
        ...
        foreach ($this->rules as $rule) {
            //超级管理员无需验证condition
            if (isset($rule['condition']) && !empty($rule['condition']) && !in_array('*', $ids)) {
                //根据condition进行验证
                $user = $this->getUserInfo($uid); //获取用户信息,一维数组
                $command = preg_replace('/\{(\w*?)\}/', '$user[\'\\1\']', $rule['condition']);
                @(eval('$condition=(' . $command . ');'));
                if ($condition) {
                    $rulelist[$rule['id']] = strtolower($rule['mca']);
                }
            } else {
                //只要存在就记录
                $rulelist[$rule['id']] = strtolower($rule['mca']);
