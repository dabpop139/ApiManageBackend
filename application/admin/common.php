...
if (!function_exists('get_tree')) {
    function get_tree($datas, $pid = 0)
    {
        ...
        foreach ($notroot as $item) {
            $lv = $item->info['pid'];
            if (!isset($arrs[$lv])) continue;
            if (!isset($arrs[$lv]->childs)) {
                $arrs[$lv]->childs = [];
            }
            $arrs[$lv]->childs[] = $item;
        }
...
if (!function_exists('build_toolbar')) {
    ...
    function build_toolbar($btns = null, $attr = [])
    {
        ...
        $btnAttr = [
            ...
            'import'  => ['javascript:;', 'btn btn-info btn-import', 'fa fa-upload', '导入', '导入'],
        ];
