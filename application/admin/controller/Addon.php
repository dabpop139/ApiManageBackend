...
use app\common\model\SubsConfig;
...
class Addon extends Backend
{
    ...
    public function config($ids = null)
    {
        ...
        // 先从数据库同步本地文件
        $dtConfig = SubsConfig::where(['group' => 'addons', 'name' => $name])->find();
        if ($dtConfig) {
            $jsonConfig = json_decode($dtConfig['content'], true);
            if (is_array($jsonConfig)) {
                //更新配置文件
                set_addon_fullconfig($name, $jsonConfig);
            }
        }
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    $result = $subsConfig->editData(['group'=>'addons', 'name'=>$name], ['content'=>json_encode($config)]);
                    if (!$result) {
                        throw new Exception('更新系统参数配置表错误');
                    }
                    //更新配置文件
